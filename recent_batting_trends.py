...
def upload_to_sheets(df):
    ...
    try:
        ...
        # Using A1 range to ensure it writes from the top-left
...
def build_hot_cold_score(df):
    ...
    df["TrendScore"] = ((df["OPS"] * 100) + (df["HR_rate"] * 50) + (df["H_rate"] * 20) + (df["BB_rate"] * 10) - (df["K_rate"] * 15)).round(2)
    return df
...
def main():
    ...
    show_cols = ["Name", "Team", "PA", "AB", "H", "HR", "RBI", "BB", "SO", "AVG", "OBP", "SLG", "OPS", "TrendScore"]
